...
- **Changes made:** `helpers/_time.py` now resolves effective cleanup settings by preferring legacy values whenever the newer split columns still match their migration defaults, `routes/chat.py` and `routes/groups.py` now use those effective settings for scheduled cleanup and banner visibility, and regression coverage was added in `tests/test_feature_drift_fixes.py`, `tests/test_chats.py`, and `tests/test_group_chats.py`.
- **Remaining risk / edge case:** Once an upgraded site explicitly customizes the split DM/group cleanup settings, those values should be treated as authoritative. A future schema migration could make that intent clearer by copying legacy values into the split columns and retiring the fallback path entirely.
